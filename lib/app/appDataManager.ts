...
export default class AppDataManager {
  ...
  private async getTargets() {
    var targets: Target[] = []
    await axios
      .post("https://flowfirmer.tingtt.jp" + `/api/getTarget`)
      .then((res) => {
                ...
                const target: Target = {
                  ...
                  outcomeSchemes: value.outcomes.map((outcomeScheme) => {
                    const res: OutcomeScheme = {
                      ...
                      unitName:
                        outcomeScheme.unitName != null
                          ? outcomeScheme.unitName
                          : "",
                      statisticsRule: outcomeScheme.statisticsRule,
                      targetValue:
                        outcomeScheme.targetValue != null
                          ? outcomeScheme.targetValue
                          : undefined,
                      defaultValue:
                        outcomeScheme.defaultValue != null
                          ? outcomeScheme.defaultValue
                          : undefined,
                    }
                    return res
                  }),
                  pinnedAtNavigationList: value.pinnedAtNavigationList
                    ? true
                    : undefined,
                  hiddenAtNavigationList: value.hiddenAtNavigationList
                    ? true
                    : undefined,
                  themeColor: {
  ...
  private async getToDos() {
    var todos: ToDo[] = []
    await axios
      .post("https://flowfirmer.tingtt.jp" + `/api/getTodoByUserId`)
      .then((res) => {
        if (res.data.status == 200) {
          const ary = res.data.data
          todos = todos.concat(
              ...
              (value: {
                ...
                const todo: ToDo = {
                  ...
                  startDatetimeScheduled:
                    value.startDatetimeScheduled != null
                      ? new Date(value.startDatetimeScheduled)
                      : undefined,
                  timeInfoExisted: value.timeInfoExisted,

                  processingTimeScheduled:
                    value.processingTimeScheduled != null
                      ? value.processingTimeScheduled
                      : undefined,

                  repeatPattern:
                    value.repeatPattern != null
                      ? value.repeatPattern
                      : undefined,
                  repeatDayForWeekly:
                    value.repeatDayForWeekly != null
                      ? value.repeatDayForWeekly
                      : undefined,

                  targetList:
                    value.targetList != null && this.targets != undefined
                      ? value.targetList
                          .map((targetId: string) =>
                            this.targets!.find(
                          ...
                          .filter(
                            (target): target is Target => target != undefined
                          )
                      : undefined,

                  term:
                    value.term != null
                      ? this.terms?.find((term) => term.id == value.term)
                      : undefined,
  ...
  private async getTerms() {
    var terms: Term[] = []
    await axios
      .post("https://flowfirmer.tingtt.jp" + `/api/getTermByUserId`)
      .then((res) => {
        if (res.data.status == 200) {
          const ary = res.data.data
          terms = terms.concat(
  ...
  private async getHabitReminds() {
    ...
    await axios
      .post("https://flowfirmer.tingtt.jp" + `/api/getHabitByUserId`)
      .then((res) => {
        if (res.data.status == 200) {
          const ary = res.data.data
          habits = habits.concat(
            ary.map(
              (value: { _id: string; name: string; target: string | null }) => {
  ...
  private getArchives() {
    // axios.post('https://flowfirmer.tingtt.jp' + `/api/getTodoArchiveByUserId`)
    //     .then((res) => {
    //         if (res.data.status == 200) {
    //             const ary = res.data.data;
    //         }
  ...
  public async registerTodo(
    ...
    await axios
      .post("https://flowfirmer.tingtt.jp" + "/api/saveTodo", {
        data: {
          name: name,
          description: description,
          startDatetimeScheduled:
            datetime != undefined ? datetime.date : undefined,
          timeInfoExisted: datetime != undefined ? datetime.timeSetted : false,
          processingTimeScheduled: processingTime,
          repeatPattern:
            repeatPattern != undefined ? repeatPattern.interval : undefined,
          repeatDayForWeekly:
            repeatPattern != undefined && repeatPattern.interval == "Weekly"
              ? repeatPattern.repeatDay
              : undefined,
          repeatDateForMonthly:
            repeatPattern != undefined &&
            datetime != undefined &&
            repeatPattern.interval == "Monthly"
              ? repeatPattern.repeatDate != undefined
                ? repeatPattern.repeatDate
        ...
        if (res.data.status == 200) {
          ...
          const newTodo: ToDo = {
            ...
            startDatetimeScheduled:
              datetime != undefined ? datetime.date : undefined,
            timeInfoExisted:
              datetime != undefined ? datetime.timeSetted : false,
            processingTimeScheduled: processingTime,
            repeatPattern:
              repeatPattern != undefined ? repeatPattern.interval : undefined,
            repeatDayForWeekly:
              repeatPattern != undefined && repeatPattern.interval == "Weekly"
                ...
                : undefined,
            repeatDateForMonthly:
              repeatPattern != undefined && repeatPattern.interval == "Monthly"
                ? repeatPattern.repeatDate
                : undefined,
            targetList:
              targetIds != undefined && this.targets != undefined
                ? this.targets.filter((target) =>
                    targetIds.some((id) => id == target.id)
                  )
                : undefined,
            term:
              termId != undefined && this.terms != undefined
                ? this.terms.find((term) => term.id == termId)
                : undefined,
            completed: completed,
          ...
          // 新規Targetを追加
          this.todos =
            this.todos != undefined ? [...this.todos, newTodo] : [newTodo]
          ret = this.todos
        } else {
    ...
    if (this.todos != undefined) {
      this.todos = this.todos.map((value) => {
        if (value.id == updatedValue.id) {
          ...
          axios
            .post(
              "https://flowfirmer.tingtt.jp" + "/api/updateTodoByObjectId",
              {
                data: {
                  _id: updatedValue.id,
                  ...
                  targetList:
                    updatedValue.targetList != undefined
                      ? updatedValue.targetList.map((target) => target.id)
                      : [],
    ...
    if (this.todos != undefined) {
      ...
      this.todos = this.todos.map((value) => {
        if (value.id == id) {
          ...
          if (
            value.completed &&
            value.startDatetimeScheduled != undefined &&
            ...
            switch (value.repeatPattern) {
              ...
              case "Weekly":
                if (
                  ...
                  value.repeatDayForWeekly.filter(
                    (dayNum) => dayNum <= 6 && dayNum >= 0
                  ).length != 0
                ) {
                  // 次の指定曜日に新規ToDo
                  do {
            ...
            targetList =
              value.targetList != undefined
                ? value.targetList?.map((target) => target.id)
                : []
  ...
  public undoToggleTodoCompletionState() {
    if (
      ...
      this.todoCompletionStateToggledTodoIds.length > 0
    ) {
      const todo = this.todos.find(
        (value) =>
          value.id ==
          this.todoCompletionStateToggledTodoIds[
            this.todoCompletionStateToggledTodoIds.length - 1
          ]
      )
      if (todo != undefined) {
  ...
  public deleteTodo(id: string) {
    if (this.todos != undefined) {
      ...
      // call api
      axios
        .post("https://flowfirmer.tingtt.jp" + "/api/deleteTodoByObjectId", {
          data: {
            _id: id,
          },
        })
  ...
  public async registerTerm(
    ...
    await axios
      .post("https://flowfirmer.tingtt.jp" + "/api/saveTerm", {
        data: {
          name: name,
          description: description,
      ...
      .then((res) => {
        if (res.data.status == 200) {
          ...
          this.terms =
            this.terms != undefined ? this.terms.concat([newTerm]) : [newTerm]
          ret = this.terms
        } else {
          console.log(res.data.message)
        }
  ...
  public async registerTarget(
    ...
    await axios
      .post("https://flowfirmer.tingtt.jp" + "/api/saveTarget", {
        data: {
          name: name,
          themeColor:
  ...
  public async registerOutcomeScheme(
    ...
    const outcomeSchemes =
      target.outcomeSchemes == undefined ? [] : target.outcomeSchemes

    let ret: OutcomeScheme | false = false

    await axios
      .post("https://flowfirmer.tingtt.jp" + "/api/saveOutcomeScheme", {
        targetId: target.id,
        outcome: {
          name: name,
          unitName: unitName,
  ...
  public registerArchive(
    ...
    if (
      refInfo.refType == "ToDo" &&
      this.archives?.some(
        (value) =>
          value.refInfo.refType == "ToDo" &&
          value.refInfo.ref.id == refInfo.ref.id
      )
    ) {
      // update
        ...
        const [positivePercent, negativePercent] = (() => {
          ...
          return feelingList
            .reduce(
              (prev, curr) => {
                return [
                  prev[0] + curr.positivePercent,
                  prev[1] + curr.negativePercent,
                ]
              },
              [0, 0]
            )
            .map((val) => val / feelingList.length)
        })()

        axios
          .post("https://flowfirmer.tingtt.jp" + "/api/saveArchive", {
            data: {
              refType: refInfo.refType,
              refId: refInfo.ref.id,
              checkInDateTime:
                refInfo.refType == "ToDo" ? refInfo.startDateTime : date,
              feelingAndDiary: {
                diaryFlag: text != undefined && text != "",
                feelingFlag:
                  feelingList != undefined && feelingList.length != 0,
                textForDiary: text,
                positiveValue: positivePercent,
                negativeValue: -negativePercent,
              },
              outcomes: outcomes
                ?.filter((outcome) => outcome.scheme.statisticsRule != "String")
                .map((outcome) => {
                  return {
        ...
        axios
          .post("https://flowfirmer.tingtt.jp" + "/api/saveArchive", {
            data: {
              ...
              refId: refInfo.ref.id,
              checkInDateTime:
                refInfo.refType == "ToDo" ? refInfo.ref.checkInDatetime : date,
              feelingAndDiary: {
                diaryFlag: text != undefined && text != "",
                feelingFlag:
                  feelingList != undefined && feelingList.length != 0,

...
export default function ArchiveExpressiveDiary(props: Props) {
        ...
        start: (() => {
            ...
            if (
                ...
                return `${props.todo.startDatetimeScheduled.getFullYear()}-${`0${
                    ...
                    -2
                )}:${`0${props.todo.startDatetimeScheduled.getMinutes()}`.slice(
                    -2
                )}`
            }
            return ""
    ...
    // 値
    const [resultDatetimeStart, setResultDatetimeStart] =
        React.useState<string>(defaultResultDatetime.start)
                                  ...
                                  value: (() => {
                                      ...
                                      if (
                                          ...
                                          const archivedOutcomes =
                                              appDataManager.archives.find(
                                                  (value) =>
                                                      value.refInfo.refType ==
                                                          "ToDo" &&
                                                      value.refInfo.ref.id ==
                                                          props.todo!.id
                                              )?.outcomes
                                          if (archivedOutcomes != undefined) {
                                              const archivedOutcome =
                                                  archivedOutcomes.find(
                                                      (outcome) =>
                                                          outcome.scheme.id ==
                                                          scheme.id
                                                  )
                                              if (
                                                  archivedOutcome != undefined
                                              ) {
                                                  defaultValue =
                                                      archivedOutcome.value
                                              }
                                          }
                                      }
                                      if (scheme.statisticsRule == "String") {
    ...
    const [
        selectOutcomeSchemeMenuAnchorEl,
        setSelectOutcomeSchemeMenuAnchorEl,
    ...
    const feels = defaultFeeingTypes.map((value) => {
        ...
        const selected = defaultFeels.find(
            (feel) => feel.feeling.id == value.id
        )
        ...
        const [positivePercent, setPositivePercent] =
            React.useState<Percentage>(defaultPositivePercent)
        const [negativePercent, setNegativePercent] =
            React.useState<Percentage>(defaultNegativePercent)
        return {
            feel: value,
            selectedState: {
                value: selectedState,
                    ...
                    <div className={classes.feelingListDiv}>
                        {feels
                            ...
                            .map((value) => {
                                ...
                                // styleを生成
                                const emotionColorClasses = makeStyles(
                                    (theme: Theme) =>
                                        createStyles({
                                            emotionColorDiv: {
                        ...
                        appDataManager.registerArchive(
                            ...
                            (() => {
                                ...
                                outcomesGroupedInTargets
                                    .filter((target) =>
                                        target.outcomes.some(
                                            (outcome) => outcome.enable
                                        )
                                    )
                                    .forEach((target) => {
                                        target.outcomes
                            ...
                            memo,
                            feels.filter((feel) => feel.selectedState.value)
                                .length > 0
                                ? feels
                                      .filter(
                                          (feel) => feel.selectedState.value
                                      )
                                      .map((feel) => {
                                          return {
                                              feeling: feel.feel,
                                              positivePercent:
                                                  feel.positivePercentState
                                                      .value,
                                              negativePercent:
                                                  feel.negativePercentState
                                                      .value,
                                          }
                                      })
                                : undefined,

import { Button, Checkbox, createStyles, Divider, List, ListItem, ListItemIcon, ListItemText, ListSubheader, makeStyles, Menu, Theme } from "@material-ui/core";
...
export default function SelectOutcomeSchemesMenu(props: Props) {
    ...
    const [newOutcomeSchemeMenuAnchorEl, setNewOutcomeSchemeMenuAnchorEl] = React.useState<HTMLElement | null>(null);
    ...
    return (
        ...
        >
            ...
            >
                    ...
                    return (
                        <li key={`section-${target.targetId}`}>
                            ...
                            {target.outcomes.map(outcome => {
                                return (
                                    <ListItem key={`item-${target.targetId}-${outcome.scheme.id}`}>
                                        <ListItemIcon>
                                            <Checkbox
                                                edge="start"
                                                ...
                                                tabIndex={-1}
                                                disableRipple
                                                inputProps={{ 'aria-labelledby': `checkbox-list-label-${outcome.scheme.id}` }}
                                            />
                                        </ListItemIcon>
                ...
                <li key={`sectin-add`}>
                    {props.outcomesState.length != 0 && <ListSubheader><Divider /></ListSubheader>}
                    <ListItem
                        button
                        onClick={(e: React.MouseEvent<HTMLElement>) => setNewOutcomeSchemeMenuAnchorEl(e.currentTarget)}
                        key={`item-add`}
                    >
                        <ListItemIcon>
                            <Add />

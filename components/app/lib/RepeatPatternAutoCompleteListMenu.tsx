...
export default function RepeatPatternAutoCompleteListMenu(props: Props) {
  ...
  const selectRepeatPattern = (
    ...
    if (key == "Daily" || key == "Monthly") {
      ...
      if (key == "Daily") {
        ...
        props.repeatPatternSetter({
          interval: key,
          repeatDate:
            props.date != null ? props.date.getDate() : newDate.getDate(),
        })
      }
    } else {
      if (key == "Weekly") {
        ...
        if (
          props.selectedRepeatPattern != null &&
          props.selectedRepeatPattern.interval == "Weekly"
        ) {
          // 既にWeeklyが指定されている場合
          props.repeatPatternSetter({
            interval: "Weekly",
            repeatDay: [...props.selectedRepeatPattern.repeatDay, dayNum],
          })
        } else {
          // 指定する曜日になるまで日付を進める
          const newDate = new Date()
    ...
    // \*~~を削除
    props.textSetter((current) =>
      current.replace(/\s+\*\w*\s*/g, "").replace(/^\*\w*\s*/g, "")
    )
    // メニューを閉じる
    closeMenu()
  }

  const intervalRepeatPatterList: ("Daily" | "Weekly" | "Monthly")[] = [
    "Daily",
    "Weekly",
    "Monthly",
  ]
  const dayRepeatPatternList: (
    | "Sun"
    | "Mon"
    | "Tue"
    | "Wed"
    ...
    >
      ...
      {intervalRepeatPatterList
        .filter((value) => {
          ...
          const str = "*".includes(inputValue.slice(0, 1))
            ? inputValue
            : undefined
        ...
        .map((value) => (
          <MenuItem onClick={() => selectRepeatPattern(value)} key={value}>
            {value}
          </MenuItem>
        ))}
      {/* 'Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat' */}
      {dayRepeatPatternList
        .filter((value) => {
          // 末尾に入力されたブロックを取り出し（スペース区切りでブロック分け）
          const inputValue = props.text.slice(props.text.lastIndexOf(" ") + 1)
          // 末尾ブロックの文字列の先頭が'*'なら変数strに保持（含まれていなければundefined）

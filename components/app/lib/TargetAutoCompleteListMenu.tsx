...
export default function DateTimeInfoSelectMenu(props: Props) {
  ...
  const selectTarget = (targetId: string) => {
    props.idListSetter((current) =>
      current == undefined ? [targetId] : current.concat([targetId])
    )
    closeMenu()
    // #~~を削除
  ...
  return (
    ...
    >
      ...
      {appDataManager.targets != undefined &&
        appDataManager.targets
          .filter((target) => {
            ...
            if (str.length == 1) {
              return !(
                props.selectedIdList != undefined &&
                props.selectedIdList.find(
                  (targetId) => targetId == target.id
                ) != undefined
              )
            }

            return !(props.selectedIdList != undefined &&
            props.selectedIdList.find((targetId) => targetId == target.id) !=
              undefined
              ? true
              : !target.name.toLowerCase().includes(str.slice(1).toLowerCase()))
          })
          .map((target) => (
            <MenuItem onClick={() => selectTarget(target.id)} key={target.id}>
              {target.name}
            </MenuItem>
          ))}
      ...
      {(() => {
        ...
        return (
          <MenuItem
            onClick={() => createNewTarget(str.slice(1))}
            key={"newTarget"}
          >
            Create new target: {str.slice(1)}
          </MenuItem>
        )
      })()}
    </Menu>

...
type Props = {
  ...
  repeatPattern:
    ...
    | null
  repeatPatternSetter: React.Dispatch<
...
export default function DateTimeInfoSelectMenu(props: Props) {
  ...
  const setDate = (e: React.ChangeEvent<HTMLInputElement> | SetDateProp) => {
    ...
    if (e.hasOwnProperty("currentTarget")) {
      if (props.endDate == null) {
        ...
        setEndDate({
          target: {
            value: `${splited[0]}-${`0${splited[1]}`.slice(-2)}-${`0${
              splited[2] + 13
            }`.slice(-2)}`,
          },
        })
  ...
  const [repeatPatternMenuAnchorEl, setRepeatPatternMenuAnchorEl] =
  ...
  return (
    ...
    >
      <div className={classes.menuInnerDiv}>
        <Tabs
        ...
        >
          <Tab className={classes.tab} label={"ToDo"} />
          <Tab className={classes.tab} label={"Term"} />
        </Tabs>
        ...
        {props.termFlg ? (
          ...
          <div>
            ...
            <div className={classes.repeatPatternSelecterDiv}>
              <Button
                className={clsx(classes.button, {
                  [classes.buttonFontColorPrimary]:
                    props.repeatPattern != undefined,
                })}
                variant="outlined"
                onClick={(e: React.MouseEvent<HTMLElement>) =>
                  setRepeatPatternMenuAnchorEl(e.currentTarget)
                }
              >
                <Loop />
                {props.repeatPattern != undefined
                  ? props.repeatPattern.interval == "Weekly"
                    ? props.repeatPattern.interval +
                      " (" +
                      props.repeatPattern.repeatDay.map(
                        (value) =>
      ...
      >
        {(
          ["Daily", "Weekly", "Monthly"] as ("Daily" | "Weekly" | "Monthly")[]
        ).map((value) => (
          <MenuItem
            ...
            onClick={() => {
              ...
              if (value == "Weekly") {
                props.repeatPatternSetter({
                  interval: value,
                  repeatDay: [
                    props.date != null ? props.date.getDay() : nowDate.getDay(),
                  ],
                })
